...
const filterRequiredTraits = (
  ...
  if (isPair)
    return Object.filter(
      assets,
      ({ traits, ...rest }) =>
        Object.keys(traits).includes(requiredTraitIndex) &&
        traits[requiredTraitIndex].includes(requiredTraitValue)
    );
  return Object.filter(assets, ({ traits, ...rest }) =>
    Object.values(traits).includes(requiredTraitIndex)
  );
};

const displayMinMaxMedianEmbed = (
  min,
  max,
  median,
  requiredTraitIndex,
  ...
  return new MessageEmbed()
    .setColor("#0099ff")
    .setTitle(
      `Min, Max and Median for ${
        requiredTraitValue
          ? `${requiredTraitIndex} : ${requiredTraitValue}`
          : `${requiredTraitIndex}`
      }`
    )
...
const fetchAndSendHighestOfferInfoForTrait = async (
  interaction,
  assets,
  contractAddress,
  collectionName,
  ...
  const filteredAssets = filterRequiredTraits(
    assets,
    requiredTraitIndex,
    requiredTraitValue,
    isPair
  ...
  if (
    forceFetch ||
    !Object.keys(filteredAssets).every((tokenId) =>
      filteredAssets[tokenId].hasOwnProperty("highestOffer")
    )
  ) {
    // Fetch
    interaction.followUp(
      `Fetching information about offers for ${enteredCollection} with trait ${
        requiredTraitValue
          ? `${requiredTraitIndex} : ${requiredTraitValue}`
          : `${requiredTraitIndex}`
      }, please waiat as this can take a long time`
    );
    ...
    for (let i = 0; i < tokenIds.length; i += 5 * 10) {
      ...
      for (const [index, response] of Object.entries(responses)) {
        const highestOfferFound = getHighestOffers(
          filteredAssets,
          response,
          tokenIdsMinorSlices[parseInt(index)]
        );
        const tokenIdsWithNoOffersFound = Object.keys(highestOfferFound).filter(
          (tokenId) => !highestOfferFound[tokenId]
        );
        const extraRequests = [];
        ...
        if (extraRequests.length > 0) {
          ...
          for (const [index, response] of Object.entries(extraResponses)) {
            filteredAssets[
              tokenIdsWithNoOffersFound[parseInt(index)]
            ].highestOffer = getSingleHighestOffer(response.data);
          }
        }
      }
    }
    ...
    for (const requiredTraitIndex in requiredTraits) {
      ...
      if (isPair) {
        for (const requiredTraitValue of requiredTraits[requiredTraitIndex]) {
          await fetchAndSendHighestOfferInfoForTrait(
            interaction,
            assets,
            contractAddress,
            collectionName,
